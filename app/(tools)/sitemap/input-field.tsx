    ...
    <>
      <div className="w-full flex justify-center flex-col items-center">
        ...
        <section className="flex gap-4 items-center">
          {loading && (
            <>
              <Skeleton className="w-[300px] dark:bg-white/50 bg-black/50" />
              <Skeleton className="w-[100px] dark:bg-white/50 bg-black/50" />
            </>
          )}
          {data.length !== 0 && (
            <>
        ...
        {data.length === 0 || error ? (
          <>
            <div className="px-10 flex items-center gap-4 py-5 text-xl bg-red-50 text-red-500 rounded-lg">
              <XCircle />
              Unable to Find Sitemap Links. Try Again
            </div>
          </>
        ) : (
          <>
            <div className="flex flex-wrap gap-3 max-w-5xl mt-8">
              {data.map((url) => (
                <Link
                  key={url}
                  href={`${baseUrl}${url}`}
                  className="text-base hover:underline underline-offset-2 dark:hover:bg-gray-100/20 hover:dark:text-white px-2 py-1 rounded-lg"
                >
                  <p>{url}</p>
                </Link>
              ))}
